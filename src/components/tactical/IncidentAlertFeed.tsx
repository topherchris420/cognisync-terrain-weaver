...
function getAlertIcon(severity: AlertSeverity) {
  switch (severity) {
    ...
    case "watch":
    case "advisory":
      return <ShieldAlert className="h-3.5 w-3.5 text-warning shrink-0 mt-0.5" />;
    default:
    ...
    <div className="flex flex-col space-y-2">
      <div className="flex items-center justify-between">
        ...
        {unacknowledgedCount > 0 && (
          <Badge variant="outline" className="text-[10px] font-mono border-destructive/50 text-destructive bg-destructive/10">
      ...
      <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin">
        {alerts.map((alert) => (
          <div
            key={alert.id}
            className={cn(
              "p-2 rounded-md border transition-all flex items-start justify-between gap-3 text-xs font-mono",
              !alert.acknowledged
          ...
          >
            ...
            {!alert.acknowledged && (
              <Button
                ...
                className="h-6 px-2 text-[10px] shrink-0 font-mono border-destructive/40 hover:bg-destructive/10 text-foreground"
              >
                Acknowledge
              </Button>
            )}
